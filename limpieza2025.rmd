...
```
```{r}
datos %>% filter(propertyCode == 106230584) %>% select(url)
```
...
```{r}
datos_pca = select(datos, - which(descProp$tipo == "binary"), - which(colnames(datos) %in% c("former_price", "price_drop_value", "price_drop_percentage", "propertyCode", "latitude", "longitude", "subTypology")))
