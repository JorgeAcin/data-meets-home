...
modelo_vars <- c(
  "CONSTRUCTEDAREA","ROOMNUMBER","BATHNUMBER","HASLIFT",
  "BUILTTYPEID_1","BUILTTYPEID_2","BUILTTYPEID_3",
...
# 1.  Quitar geometría
Valencia_Sale_nogeo <- Valencia_Sale %>% st_drop_geometry()

# 2.  Resumen por distrito-rango
resumen <- Valencia_Sale_nogeo %>% 
  group_by(DISTRITO, RANGO_PRECIO) %>% 
  summarise(across(all_of(modelo_vars), mean, na.rm = TRUE),
            .groups = "drop")

# 3.  Pivotar: una fila por distrito
resumen_distritos <- resumen %>% 
  pivot_wider(
    names_from  = RANGO_PRECIO,                 # Barato / Medio / Caro
    values_from = all_of(modelo_vars),
    names_glue  = "{.value}_{RANGO_PRECIO}",
    values_fill = NA                            # <- NA si falta el rango
  )
...
```
Hemos creado un data frame paralelo en el que contamos con todos los distritos. Las columnas de este son la media de las variables seleccionadas (coincidentes con 2025), divididas en distintos rangos de precio.
...
summary(hop_val)
```
Observamos el valor del estadístico de Hopkins que supera por poco el 0,5 , y su valor maáximo llega a los 0,6640. Esto indica una débil o moderada tendencia al agrupamiento.
...
```{r}
set.seed(111)
clust3 <- kmeans(coords_pca, centers = 6, nstart = 20)
...
Cluster 1 (azul, PC1 negativas)

Comprende los distritos más céntricos y de precio alto:
Ciutat Vella, L'Eixample, El Pla del Real y Extramurs.

Están muy próximos entre sí en el espacio PCA, lo que indica que comparten características muy similares (precio m² elevado, corta distancia al centro/metro, ascensor habitual,…).

La dispersión en PC2 es reducida: todos ellos tienen configuraciones de habitaciones/baños parecidas.

Cluster 2 (amarillo, PC1 positivas)

Agrupa al resto de distritos más periféricos o “de precio moderado‐bajo”:
p. ej. Benicalap, Rascanya, Jesús, Patraix, Pobles del Nord, etc., incluyendo el muy singular Natzaret.

En PC1 se separan claramente de los urbanos caros, señalando una menor presión de precio y mayor distancia.

En PC2 muestran más variabilidad: algunos son más grandes (más habitaciones/baños), otros más pequeños.

Conclusiones clave
PC1 (33,5 %) capta sobre todo la tensión precio–proximidad: valores negativos → caro y céntrico; positivos → más barato y periférico.

PC2 (15,5 %) refleja diferencias en volumen y comodidades (nº de habitaciones/baños).

El k-means recupera dos segmentos muy intuitivos:

“Premium central” (cluster 1)

“Residencial/periférico” (cluster 2)

La baja superposición de elipses en el gráfico sugiere que la partición es sólida: cada grupo es compacto y bien separado.

Natzaret aparece como un outlier dentro del cluster 2 (PC1 y PC2 muy altos), lo que indica un comportamiento atípico incluso dentro de las zonas más “baratas” (quizá por tamaño o características excepcionales).

Estos insights te permiten perfilar ofertas de producto o estrategia comercial bien diferenciadas para cada segmento de distrito.
...
# 1) Data.frame con distrito ↔ cluster
distr_cluster <- data.frame(
  DISTRITO = rownames(coords_pca),
  CLUSTER  = clust3$cluster
)

# 2) Lista de distritos por cluster
split(distr_cluster$DISTRITO, distr_cluster$CLUSTER)
```
